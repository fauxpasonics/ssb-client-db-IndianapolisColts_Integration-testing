...
WHERE SSB_CRMSYSTEM_Contact_ID IN (
	SELECT SSB_CRMSYSTEM_Contact_ID FROM dbo.vwDimCustomer_ModAcctId a
		WHERE SSB_CRMSYSTEM_Contact_ID IN (SELECT DISTINCT SSB_CRMSYSTEM_Contact_ID FROM dbo.vwDimCustomer_ModAcctId WHERE SourceSystem LIKE '%SFDC_Account%') --updateme
