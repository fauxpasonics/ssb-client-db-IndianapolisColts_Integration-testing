...
FROM dbo.Account b 
--LEFT JOIN ProdCopy.[vw_Account] b ON a.[SSB_CRMSYSTEM_ACCT_ID] = b.[SSB_CRMSYSTEM_ACCT_ID__c]
...
WHERE SSB_CRMSYSTEM_ACCT_ID IN (
	SELECT SSB_CRMSYSTEM_ACCT_ID FROM dbo.vwDimCustomer_ModAcctId a
		WHERE SSB_CRMSYSTEM_ACCT_ID IN (SELECT DISTINCT SSB_CRMSYSTEM_ACCT_ID FROM dbo.vwDimCustomer_ModAcctId WHERE SourceSystem LIKE '%SFDC_Account%') --updateme
...
WHERE SSB_CRMSYSTEM_ACCT_ID IN (
	SELECT SSB_CRMSYSTEM_ACCT_ID FROM dbo.vwDimCustomer_ModAcctId a
		WHERE SSB_CRMSYSTEM_ACCT_ID IN (SELECT DISTINCT SSB_CRMSYSTEM_ACCT_ID FROM dbo.vwDimCustomer_ModAcctId WHERE SourceSystem LIKE '%SFDC_Contact%') --updateme
	GROUP BY SSB_CRMSYSTEM_ACCT_ID
HAVING COUNT(DISTINCT SourceSystem) = 1
) 
AND LoadType = 'Upsert'
