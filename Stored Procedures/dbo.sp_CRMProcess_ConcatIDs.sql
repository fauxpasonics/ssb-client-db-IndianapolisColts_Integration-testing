...
UPDATE a
SET a.SSB_CRMSYSTEM_SSID_TIX__c = ISNULL(LTRIM([b].[ConcatIDs1]),'')
...
INNER JOIN stg.[tbl_CRMProcess_ConcatIDs] b ON a.SSB_CRMSYSTEM_ACCT_ID__c = b.GUID

IF @ObjectType = 'Contact'
UPDATE a
SET a.SSB_CRMSYSTEM_SSID_TIX__c = ISNULL(LTRIM([b].[ConcatIDs1]),'')
, [AccountId] = ISNULL(LTRIM([b].[ConcatIDs2]),'')
, a.SSB_CRMSYSTEM_DimCustomerID__c = ISNULL(LTRIM([b].[DimCust_ConcatIDs]),'')
--, GP_Ids = ISNULL(LTRIM([b].[DimCust_ConcatIDs]),'')
-- SELECT b.* 
